...
foreach ( $items as $item_id => $item ) :
    ...
    $i++;
    $purchase_note = $show_purchase_note && is_object( $_product ) && ( $purchase_note = get_post_meta( $_product->id, '_purchase_note', true ) );
	...
	if ( apply_filters( 'woocommerce_order_item_visible', true, $item ) ) {
		...
		<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order ) ); ?>">
			...
			?></td>
			<td class="yith-wcet-order-items-table-element-quantity <?php echo $last_class; ?>" style="vertical-align:middle;"><?php echo apply_filters( 'woocommerce_email_order_item_quantity', $item['qty'], $item ); ?></td>
			<td class="yith-wcet-order-items-table-element-quantity <?php echo $last_class; ?>" style="vertical-align:middle;"><?php echo $order->get_formatted_line_subtotal( $item ); ?></td>
		</tr>
		<?php
	}
	...
	if ( $show_purchase_note && is_object( $_product ) && ( $purchase_note = get_post_meta( $_product->id, '_purchase_note', true ) ) ) :
        ...
        ?>
		<tr>
			<td  class="yith-wcet-order-items-table-element <?php echo $last_class; ?>" colspan="3" style="text-align:left; vertical-align:middle;"><?php echo wpautop( do_shortcode( wp_kses_post( $purchase_note ) ) ); ?></td>
		</tr>
	<?php endif; ?>
