	...
	*/
	...
	<script type="text/javascript">	
		...
		jQuery(function($){
			...
			// delete file
			$("#nm-uploader-area-<?php echo $args['id']?>").on('click','.u_i_c_tools_del', function(e){
				e.preventDefault();
			...
			uploaderInstances['<?php echo $args['id']?>'] = uploader_<?php echo $args['id']?>;
...
<?php
		...
		if ($args ['photo-editing'] == 'on' && $args['aviary-api-key'] != '') {
			...
			$crop_preset = $args['cropping-ratio'];
			$js_crop_preset = '';
			if($crop_preset){
				$js_crop_preset = '[';
			...
			echo 	'editing_tools = (editing_tools == "" && editing_tools == undefined) ? \'all\' : editing_tools;';
				echo 'featherEditor.launch({';
					...
					if($js_crop_preset != '')
						echo 'cropPresets: '.$js_crop_preset.',';
