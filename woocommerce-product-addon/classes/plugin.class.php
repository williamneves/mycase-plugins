...
<?php
	...
	function product_meta_box($post) {
		...
		foreach ( $all_meta as $meta ) {
			...
			echo '<option value="' . $meta->productmeta_id . '" ' . $selected . ' id="select_meta_group-' . $meta->productmeta_id . '">';
		...
		if ($existing_meta) {
			foreach ( $existing_meta as $meta ) {
				...
				if ($meta->type == 'checkbox') {
					...
					$validate_value = (isset($_POST[$validate_name]) ? $_POST[$validate_name] : '');
	...
	function add_fixed_fee($cart_object) {
		...
		foreach ( $cart_object->cart_contents as $key => $value ) {
			...
			if( !isset( $value['product_meta']) )
				return;	    
		    //nm_personalizedproduct_pa($value['product_meta']);
	...
	function get_option_price(){
		...
		$optionprices = (isset($_REQUEST['optionprices']) ? $_REQUEST['optionprices'] : '');
		if($optionprices){
			foreach ($optionprices as $pair){
				...
				if(strpos($price,'%') !== false){
					...
					$html .= $option . ' ('.$percent_price.') ' . woocommerce_price($price) . '<br>';
				}else{
	...
	function uploaded_html($file_dir_path, $file_name, $is_image, $settings){
		...
		$_html .= '<td style="vertical-align:middle"><img data-filename="'.$file_name.'" id="'.$file_id.'" src="'.$thumb_url.'" />';
