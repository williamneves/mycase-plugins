...
$current_icon = YITH_WCCOS_ICON()->get_icon_data( $current_options['icon'] );
$std_icon = YITH_WCCOS_ICON()->get_icon_data( $std['icon'] );
...
<div id="<?php echo $id ?>-container" <?php if ( isset( $deps ) ): ?>data-field="<?php echo $id ?>" data-dep="<?php echo $deps['ids'] ?>" data-value="<?php echo $deps['values'] ?>" <?php endif ?>class="select_icon rm_option rm_input rm_text">
    ...
    <div class="option">
        <div class="select_wrapper yith_wccos_icon_list_type clearfix">
        ...
        <div class="yith-wccos-icon-manager-wrapper">
            <div class="yith-wccos-icon-manager-text">
                <div class="yith-wccos-icon-preview" <?php echo  $current_icon ?>></div>
                <input type="text" id="<?php echo $id ?>[icon]" class="yith-wccos-icon-text" name="<?php echo $name ?>[icon]" value="<?php echo $current_options['icon']; ?>" />
            </div>


            <div class="yith-wccos-icon-manager">
                <ul class="yith-wccos-icon-list-wrapper">
                    <?php foreach ( $options['icon'] as $font => $icons ):
                        foreach ( $icons as $key => $icon ): ?>
                            <li data-font="<?php echo $font  ?>" data-yicon="<?php echo ( strpos( $key , '\\') === 0 ) ? '&#x'.substr( $key , 1 ) : $key  ?>" data-key="<?php echo $key ?>" data-name="<?php echo $icon ?>"></li>
                        <?php
                        endforeach;
                    endforeach; ?>
                </ul>
...
<script>
    ...
    jQuery(document).ready( function($){
        ...
        $('.select_wrapper.yith_wccos_icon_list_type').on('change', function(){

            var t       = $(this);
            var parents = $('#' + t.parents('div.select_icon').attr('id'));
            var option  = $('option:selected', this).val();
            var to_show = option == 'none' ? '' : option == 'icon'  ? '.yith-wccos-icon-manager-wrapper' : '.custom_icon_wrapper';

            parents.find('.option > div:not(.yith_wccos_icon_list_type)').removeClass('show').addClass('hidden');
            parents.find( to_show ).removeClass( 'hidden' ).addClass( 'show' );
        });

        $('.select_wrapper.yith_wccos_icon_list_type').trigger('change');

        var $icon_list = $('.select_icon').find('ul.yith-wccos-icon-list-wrapper'),
            $preview = $('.yith-wccos-icon-preview'),
            $element_list = $icon_list.find('li'),
            $icon_text = $('.yith-wccos-icon-text');
        ...
        $element_list.on("click", function () {
            ...
            $preview.attr('data-yicon', $t.data('yicon'));
