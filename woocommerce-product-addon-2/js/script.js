...
jQuery(function($){
	...
	setTimeout(function(){
		...
		// setting up prices range
		if ($(".single_variation .price .amount").length > 0){
			$price = $(".single_variation .price");
		}else if($('p[itemprop="price"]').closest('.price').find('.amount').length > 0){
			$price = $('p[itemprop="price"]').closest('.price');
		}else{
			$price = $('.amount').closest('.price');
		}
		...
		if(price_matrix != '' && price_matrix != undefined){
			...
			$price.find('.amount').hide();
			var post_data = {action: 'nm_personalizedproduct_set_matrix_price',
					matrix: price_matrix};
