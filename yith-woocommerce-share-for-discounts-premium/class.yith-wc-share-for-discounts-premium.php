...
if ( ! class_exists( 'YITH_WC_Share_For_Discounts_Premium' ) ) {
	...
	class YITH_WC_Share_For_Discounts_Premium extends YITH_WC_Share_For_Discounts {
		...
		public function __construct() {
			...
			if ( get_option( 'ywsfd_enable_plugin' ) == 'yes' ) {
				...
				if ( is_admin() ) {
					...
					add_filter( 'ywsfd_social_buttons', array( $this, 'ywsfd_social_buttons' ), 10, 2 );
		...
		public function frontend_scripts_premium() {
			...
			wp_enqueue_script( 'ywsfd-frontend-premium', YWSFD_ASSETS_URL . '/js/ywsfd-frontend-premium' . $suffix . '.js', array( 'jquery', 'ywsfd-frontend' ) );
		 ...
		 * @author  Alberto Ruggiero
		 */
		public function ywsfd_social_buttons( $value = '', $social_params ) {

			return include( YWSFD_TEMPLATE_PATH . '/frontend/social-buttons-premium.php' );
		 ...
		 * @author  Alberto Ruggiero
		 */
		public function ywsfd_available_socials() {
		...
		public function ywsfd_post_url( $url ) {
			...
			if ( get_option( 'ywsfd_custom_url' ) != '' ) {
				...
				$url = get_option( 'ywsfd_custom_url' );
