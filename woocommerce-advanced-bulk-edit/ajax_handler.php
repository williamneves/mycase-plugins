			...
			foreach($custsearchparam as $custitem)
			{
				if(isset($custitem['type']) && ($custitem['type'] !== 'custom' && $custitem['type'] !== 'customh' && $custitem['type'] !== 'attribute'))
				{
					if($custitem['type'] === "date")
						continue;
					if($custitem['id'] == "ID" || $custitem['id'] == "_stock" || $custitem['id'] == "_stock_status" || $custitem['id'] == "post_status")
					{
...
//							AND CASE WHEN meta{$innercounter}.meta_key='{$custitem['id']}' THEN meta{$innercounter}.meta_value
							$attrsquery = " INNER JOIN {$meta} ON p1.ID={$meta}.post_id AND ";
							$attrsquery.= "({$meta}.meta_key='attribute_pa_".$custitem['title']['attr']."' AND {$meta}.meta_value='".$custitem['title']['value']."')";
//							$attrsquery.= "( CASE WHEN {$meta}.meta_key='attribute_pa_".$custitem['title']['attr']."' THEN {$meta}.meta_value='".$custitem['title']['value']."' ";
						}else
						{
							$attrsquery.= " OR ({$meta}.meta_key='attribute_pa_".$custitem['title']['attr']."' AND {$meta}.meta_value='".$custitem['title']['value']."')";
...
//						$gal_thumbids = explode(',',$id->_product_image_gallery);
						$query ="SELECT post_id,meta_value
						FROM  {$meta} WHERE post_id IN ({$id->_product_image_gallery}) AND meta_key='_wp_attachment_metadata'";
						$metathumbs =  $wpdb->get_results($query);
						if ( false === $metathumbs) {
							continue;
						}
...
//					$attrmapslugtoname = array();
//					$attributes = array();
					self::GetAttributes($attributes,$attrmapslugtoname,true,$attrname);
				}
				$attvalue = $arrrow['value'];
				$attvalue = str_replace('\"','"',$attvalue);
				$attvalue = str_replace("\'","'",$attvalue);
...
//						self::HandleAttrs($post_id,$parentid,$parentattrs_cache,$attributes,$attvalue,$attrname,count($varrow),$update_parent_attr,true,true);
	...
	public static function deleteProducts(&$data,$type,$deleteinternal = false)
	{
		global $wpdb;
		$posts = $wpdb->posts;
		$meta = $wpdb->postmeta;
...
//					$prod->ID = $prodid;
			...
			case 'deleteproducts':
			{
				...
				if(is_array($curr_settings))
				{
					if(isset($curr_settings['deleteinternal']))
					{
						if($curr_settings['deleteinternal'] === "1")
						{
							$deleteinternal = true;
						}
					}
				}
				self::deleteProducts($newarr,$deltype,$deleteinternal);
			}break;
			case 'savecolumns':
			{
				if(!empty($data))
